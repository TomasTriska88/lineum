...
with open(os.path.join(output_dir, "radius_log.csv"), "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["step", "avg_radius"])
    writer.writerows(radius_log)

with open(os.path.join(output_dir, "vortex_log.csv"), "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["step", "num_pos", "num_neg", "net_charge"])
    writer.writerows(vortex_log)

with open(os.path.join(output_dir, "particle_log.csv"), "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["step", "center_y", "center_x", "size"])
    writer.writerows(particle_log)

with open(os.path.join(output_dir, "interaction_log.csv"), "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(
        ["step", "vortices_pos", "vortices_neg", "net_local_charge"])
    writer.writerows(interaction_log)

with open(os.path.join(output_dir, "amplitude_log.csv"), "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["step", "central_amplitude"])
    writer.writerows(amplitude_log)

with open(os.path.join(output_dir, "phi_center_log.csv"), "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["step", "phi_center_abs"])
    writer.writerows(phi_center_log)
...
# Uložení do CSV
with open(os.path.join(output_dir, "spectrum_log.csv"), "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["frequency_Hz", "amplitude"])
    writer.writerows(zip(positive_freqs, positive_spectrum))

with open(os.path.join(output_dir, "trajectories.csv"), "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["id", "step", "y", "x", "amplitude"])
...
# Uložení
with open(os.path.join(output_dir, "multi_spectrum_log.csv"), "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["y", "x", "dominant_freq_Hz", "energy_J"])
    writer.writerows(multi_spectrum_rows)

with open(os.path.join(output_dir, "topo_log.csv"), "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["step", "num_pos", "num_neg",
                    "net_charge", "total_vortices"])
    writer.writerows(topo_log)
...
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "spectrum_plot.png"))
plt.close()
...
ani.save(os.path.join(output_dir, "lineum_flow.gif"),
...
ani.save(os.path.join(output_dir, "lineum_full_overlay.gif"),
         writer=PillowWriter(fps=10))
plt.close(fig)

# 🌀 Uložení všech polí vírů do souboru pro analýzu
frames_vort_np = np.array(frames_vort)  # shape: (steps, size, size)
np.save(os.path.join(output_dir, "frames_vortices.npy"), frames_vort_np)
