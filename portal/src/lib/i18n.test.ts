...
describe('Portal i18n Dictionary Parity', () => {

    // Recursive function to get all flattened key paths (e.g., ['nav.portal', 'nav.lab'])
    ...
    const checkCoverage = (targetLang: string, keys: string[]) => {
        const missing = enKeys.filter(key => !keys.includes(key));
        expect(missing, `Missing translations in ${targetLang}: \n${missing.join('\n')}`).toEqual([]);

        const orphans = keys.filter(key => !enKeys.includes(key));
