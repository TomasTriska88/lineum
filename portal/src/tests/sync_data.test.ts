...
describe('Data Synchronization', () => {
    // Only run this suite if we can access the source directories (e.g. not in a limited CI environment without full repo)
    // But for safe-merge, we expect full repo.
    ...
    afterAll(() => {
        ...
        // We might want to run sync again to remove the artifact from target, but it's safe to leave for next sync
        // actually, let's clean up the target too to be nice
        const targetFile = path.join(TARGET_DIR, TEST_FILE_NAME);
        if (fs.existsSync(targetFile)) fs.unlinkSync(targetFile);

        // We won't modify ai_index.json back, it will resolve on next dev run or sync
    });

    it('should copy new files from source to target', () => {
        // Run sync-data.js
        console.log('Running sync-data.js...');
        try {
            execSync('node scripts/sync-data.js', { cwd: PROJECT_ROOT, stdio: 'inherit' });
        } catch (e) {
            throw new Error(`Sync script failed: ${e.message}`);
        }
