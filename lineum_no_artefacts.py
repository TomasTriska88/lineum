...
output_dir = "output_no_artefacts"
...
def evolve(psi, delta, phi, kappa):
    ...
    phi += kappa * reaction_strength * (local_input - phi) * 0.5

    phi += kappa * 0.02 * diffuse_complex(phi)

    phi += 0.01 * diffuse_complex(diffuse_complex(phi))
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(steps), desc="Processing steps", unit="step"):
        ...
        phi_min = np.min(np.abs(phi))
        phi_range_log.append((i, phi_min, phi_max))
    ...
    save_csv("phi_range_log.csv", [
             "step", "phi_min", "phi_max"], phi_range_log)
