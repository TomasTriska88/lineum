...
This file contains an overview of research points that require further testing, visualization, or quantitative verification. Each point should be either (re)verified by simulation or explicitly formulated as a hypothesis. The state of this TODO is aligned with the core paper **lineum-core v1.0.19** (Eq-4, static κ, 2D, periodic BCs, RUN_TAG `spec6_false_s41`).
...
## 🔍 Phenomena from core paper to revalidate (core v1.0.19)
...
- [ ] (Katina's [HYPOTHESIS]) Explore the scenario of a **multilayer Lineum** ("several layers of Lineum stacked together"), where
      ...
      index `n` in the equations; in documentation, explicitly separate this from core v1.0.19.
...
- [ ] **[TECHNICAL DEBT] Migrate to NumPy 2.0+ and establish a new Canonical Baseline.**
       - **Context:** `lineum-core v1.0.19` strictly pins `numpy<2.0.0` (specifically relying on 1.25.x math) to preserve the exact floating-point rounding math used to generate the initial `reference_manifest_spec6_false_s41.json`. Upgrading to NumPy 2.x introduces microscopic LSB deviations in float64 arrays (due to new C-level AVX512/SIMD optimizations) that accumulate over thousands of PDE steps, inevitably changing the final SHA-256 hash of the simulation state.
       ...
       - **Action 1 (The Upgrade - Target v1.0.19):** Once the Core Paper is finalized/published, unpin NumPy in `requirements.txt` and allow the ecosystem to upgrade to NumPy 2.x and SciPy 1.14+.
       ...
       - **Action 3 (Documentation):** Generate the new JSON manifest, commit it, and explicitly state in the release notes / whitepaper that `v1.0.19` represents a "Canon Break" and that all future reproducibility tests must be run against NumPy 2.x.
...
- [ ] Clearly define the **scope relative to Lineum**:
       ...
       – state that any potential mapping onto Lineum (ψ, φ, κ, linons, Structural Closure) is an **interpretation beyond the core model**, not part of lineum-core v1.0.19.
...
- **Preliminary Verification (Local Run 2026-02-22):**
    ...
    - **Result:** The baseline Lineum engine (v1.0.19) currently distributes flow highly homogeneously. The top $20\%$ of active cells carry exactly $\sim20.01\%$ to $21.04\%$ of the total system volume. 
...
- [ ] **4) (License) Future public release of Lineum (new version): migrate core to AGPL**
    - *(Note: historical Zenodo v1.0.19 is MIT and will remain so as a proof of priority).*
...
- [ ] **M.4 Marketing (Drops & Gamification)**
    - Present products as limited "Drops" (e.g., v1.0.19 Founder's edition) to foster exclusivity.
